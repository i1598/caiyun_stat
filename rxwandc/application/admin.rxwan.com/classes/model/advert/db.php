...
class Model_DB extends Model {
	...
	public function __construct( $config = array( ) ){
		...
		$this->_key   = Arr::get( $config, "key", "id" );
	}
	...
	/*关联操作的*/
	public function getJoinList( $conditions, $limit=NULL, $offset=NULL, $orders=array() ){
	...
	public function getJoinCount( $conditions, $distict = "" ){
		$distict = empty( $distict ) ? "*" : "DISTINCT '".$distict."' ";
		$query = $this->_query( $conditions )
				 ->select( array(DB::expr("COUNT(".$distict.")"), 'totalCount'));
		return $this->_exe( $query )->get( "totalCount" );
	...
	private function _query( $conditions){
		$query = DB::select($this->_table.".*",$this->_join.".soft_name")->from( $this->_table)->join($this->_join )->on($this->_table.'.soft_id','=',$this->_join.'.soft_id');
		if( empty( $conditions ) ) return $query;
		foreach( $conditions as $key=>$value ){
			$key = $this->_table.".".$key;
			if( is_array( $value ) )
