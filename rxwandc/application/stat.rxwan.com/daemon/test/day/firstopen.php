...
function report($dateline, $member_id,$soft_id, $quantity){
    if($quantity>0){ 
        ...
        $data = array();
        $data['quantity_firstopen'] = $quantity;
...
function get_total_one($table_sn, $start=0, $end=0, $source1='',$source2=''){
	...
	$sql = "SELECT COUNT(DISTINCT `uuid`) FROM `firstopen`.`firstopen_$table_sn` WHERE `dateline`>='$start' AND `dateline`<'$end' $sql_attach";
