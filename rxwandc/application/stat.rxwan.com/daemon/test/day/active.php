...
echo "starting soft_active stat.\n";
...
function get_total($start, $end, $source1='',$source2=''){
    ...
    for($d=0;$d<16;$d++){
        for($i=0;$i<256;$i++){    
            $total += get_total_one($d, $i, $start, $end, $source1,$source2);
        }
        echo '-';
    }
...
function get_total_one($db_sn,$table_sn, $start=0, $end=0, $source1='',$source2=''){
    $sql_attach = '';
    if(!empty($source1)){
    ...
    $sql = "SELECT COUNT(DISTINCT `uuid`) FROM `active_$db_sn`.`active_$table_sn` WHERE `dateline`>='$start' AND `dateline`<'$end' $sql_attach";
