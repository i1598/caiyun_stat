...
function report($dateline, $member_id, $quantity){
    if($quantity>0){ 
        ...
        $data = array();
        $data['quantity_install_firstopen'] = $quantity; 
...
function get_total_one($table_sn, $start=0, $end=0, $source=''){
    ...
    $sql = "SELECT COUNT(DISTINCT `uuid`) FROM `install2`.`install_$table_sn` WHERE `dateline`>='$start' AND `dateline`<'$end' AND `is_firstopen`=1 $sql_attach";
