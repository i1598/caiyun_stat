...
function report($dateline, $member_id, $quantity){
    if($quantity>0){ 
        ...
        $data = array();
        $data['quantity_active_2'] = $quantity;
...
function get_total_one($db_sn, $table_sn, $start=0, $end=0, $source=''){
    ...
    $sql = "SELECT COUNT( * ) AS total FROM (SELECT count( `uuid` ) AS cnt FROM `active2_$db_sn`.`active_$table_sn` WHERE `dateline`>='$start' AND `dateline`<'$end' $sql_attach GROUP BY uuid HAVING cnt >=2)A";
