...
// 86400*21 = 1814400;
// 86400*30 = 2592000;
$livetime_start = 1814400;
$livetime_end = 2592000;

// 首先统计所有的
echo "start total: ".date("r")."\n";
$total = get_total($pos_start, $pos_end, '', $livetime_start, $livetime_end);
...
if($total_rows){ //有记录，请导入临时表        
    while(($row = fetch_assoc($query,'alliance'))!==FALSE){  
    	if ($row['dateline']<$today_start) {
    		$total_coop = get_total($pos_start, $pos_end, $row['username'],$livetime_start, $livetime_end);
...
function report($dateline, $member_id, $quantity){
    if($quantity>0){ 
        ...
        $data = array();
        $data['quantity_uninstall_month'] = $quantity;
...
function get_total($start, $end, $source='', $livetime_start=0, $livetime_end=86400){
    $total = 0;
    for($i=0;$i<64;$i++){    
        $total += get_total_one($i, $start, $end, $source, $livetime_start, $livetime_end);
    }
    return $total;
}
...
function get_total_one($table_sn, $start=0, $end=0, $source='', $livetime_start=0, $livetime_end=86400){
    $sql_attach = '';
    if(!empty($source)){
        $sql_attach .= " AND coop='$source'";
    }
    $sql_attach .= " AND livetime>=$livetime_start AND livetime<$livetime_end";
