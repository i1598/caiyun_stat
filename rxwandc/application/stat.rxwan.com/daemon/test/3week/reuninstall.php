...
function report($dateline, $member_id, $quantity_reuninstall){
    if($quantity_reuninstall>0){ 
        $table = 'report_3week';
        ...
        $data = array();
        $data['quantity_reuninstall'] = $quantity_reuninstall;
	...
	if (IS_DEBUG) {
    	echo "<M:$member_id,D:$quantity_reuninstall>--";
    }
...
//这里需要在一定时间段内再次  DISTINCT 进行排重了 (非整个时期的排重)
function get_total_one($table_sn, $start=0, $end=0, $source=''){
    $sql_attach = '';
    if(!empty($source)){
        $sql_attach .= " AND coop='$source'";
    }
    $sql = "SELECT COUNT(DISTINCT `uuid`) FROM `reuninstall2`.`uninstall_$table_sn` WHERE `dateline`>='$start' AND `dateline`<'$end' $sql_attach";
