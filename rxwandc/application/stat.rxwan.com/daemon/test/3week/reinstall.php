...
function report($dateline, $member_id, $quantity){
    if($quantity>0){ 
        ...
        $data = array();
        $data['quantity_reinstall'] = $quantity;
...
function get_total_one($table_sn, $start=0, $end=0, $source=''){
    ...
    $sql = "SELECT COUNT(DISTINCT `uuid`) FROM `reinstall2`.`reinstall_$table_sn` WHERE `dateline`>='$start' AND `dateline`<'$end' $sql_attach";
